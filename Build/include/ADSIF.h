...
private:
	...
	void projectToolTipPoints(const toolpathZHeightProps &masterHeightProps, toolpathZHeightProps &tempHeightPropsSaved);
	void reverseAllToolpath(toolpathZHeightProps &masterHeightProps);
	...
	static void findClosestPntsBetweenContours(const std::vector<gp_Pnt> &contourPntsIn, const std::vector<gp_Pnt> &pntsToSearch, std::vector<gp_Pnt> &closestPntsOut);
