...
bool TPIF::initContactPnts(amplToolpath &myCurrentToolpathIn, TopoDS_Compound &myCurrentTopContactPntsIn, TopoDS_Compound &myCurrentBotContactPntsIn,
...
bool TPIF::initToolTipPnts(amplToolpath &myCurrentToolpathIn, TopoDS_Compound &myCurrentTopToolTipsIn, TopoDS_Compound &myCurrentBotToolTipsIn,
...
bool TPIF::calcContactPnts()
{
	...
	textEdit->insertPlainText(QString("Initiating TPIF toolpath algorithm. Toolpath rendering will be updated periodically.\n"));
...
bool TPIF::calcToolTipPnts(const bool checkToolpathInterference, const int performConstZHeightTips)
{
...
bool TPIF::toolTipPreprocessing(toolpathZHeightProps &masterHeightProps)
{
...
void TPIF::getToolTipPoints(const toolpathZHeightProps &masterHeightProps, toolpathZHeightProps &tempHeightPropsSaved, bool checkForInterference, int performConstZHeightTips)
{
	...
	{
		// Next, the tool tips for the bottom tool
	...
	if (performConstZHeightTips == 0) // Only valid to calculate tool gaps if tool-tips have not been modified by constant Z-height algorithm
	{
		...
		if (directionDown)
		{
			...
			{
				gp_Pnt curTipPnt = topToolTipPnts.at(m);
				//gp_Pnt curToolCenterPnt = topToolCenterPnts.at(m);
				gp_Pnt curDieContactPnt = botContactPnts.at(m);
				float curToolGap;

				if (masterHeightProps.localAmplToolpath.topToolProfile == amplToolpath::toolProfile::hemispherical)
					curToolGap = (topToolCenterPnts.at(m)).Distance(curDieContactPnt) - (topToolDiam / 2.0);
				else if (masterHeightProps.localAmplToolpath.topToolProfile == amplToolpath::toolProfile::flatTop)
					curToolGap = (topToolCornerCenterPnts.at(m)).Distance(curDieContactPnt) - (topToolCornerRad);
		...
		{
			for (int m = 0; m < botToolTipPnts.size(); m++)
			{
				gp_Pnt curTipPnt = botToolTipPnts.at(m);
				//gp_Pnt curToolCenterPnt = botToolCenterPnts.at(m);
				gp_Pnt curDieContactPnt = topContactPnts.at(m);
				float curToolGap = 0.0;

				if (masterHeightProps.localAmplToolpath.botToolProfile == amplToolpath::toolProfile::hemispherical)
					curToolGap = (botToolCenterPnts.at(m)).Distance(curDieContactPnt) - (botToolDiam / 2.0);
				else if (masterHeightProps.localAmplToolpath.botToolProfile == amplToolpath::toolProfile::flatTop)
					curToolGap = (botToolCornerCenterPnts.at(m)).Distance(curDieContactPnt) - (botToolCornerRad);
...
void TPIF::refreshGraphics(const toolpathZHeightProps &allHeightProps, bool refreshContactPnts, bool refreshToolTipPnts)
{
...
void TPIF::removeCollinearPnts(Standard_Real relAngleTolDeg, std::vector<gp_Pnt> &orderdPntsArrInOut, std::vector<int> &indicesKept)
{
...
bool TPIF::toolpathPreprocessing(toolpathZHeightProps &masterHeightProps, std::vector<float> &allZDepthSlices)
{
	float unAssignedVal = myCurrentToolpath->getUnassignedVal();
	if (myCurrentToolpath->topContactPntsActive())
...
bool TPIF::findNextAdaptiveZSlice(const toolpathZHeightProps &tempHeightPropsCurrent, const toolpathZHeightProps &masterHeightProps, float &nextZHeightOut)
{
...
bool TPIF::getDiscretizedIntersection(ampl::planeIntersecter &contourObj, toolpathZHeightProps &tempHeightPropsCurrent, const toolpathZHeightProps &masterHeightProps)
{
	float currentHeight = tempHeightPropsCurrent.currentZDepth;
...
void TPIF::calcSurfInfo(ampl::planeIntersecter &contourObj, toolpathZHeightProps &tempHeightPropsSaved, toolpathZHeightProps &masterHeightProps)
{
	bool directionDown = false;
	if (masterHeightProps.localAmplToolpath.myFormingDirectionType == amplToolpath::formingDirectionType::negZDir)
		directionDown = true;
	...
	if (directionDown)
	{
		...
		tempHeightPropsSaved.pushBackZDepthBotContactPnts(false, false);
		tempHeightPropsSaved.pushBackZDepthTopContactPnts(false, true);
		tempHeightPropsSaved.pushBackZDepthContactPntsSurfProps(false, 0);
	}
	else
	{
		...
		tempHeightPropsSaved.pushBackZDepthBotContactPnts(false, true);
		tempHeightPropsSaved.pushBackZDepthContactPntsSurfProps(false, 0);
	}
	...
	tempHeightPropsSaved.pushBackAllSurfPropsToOther(masterHeightProps, 0, false);
}

void TPIF::calcApproxSpiralSurfInfo(ampl::planeIntersecter &contourObj, toolpathZHeightProps &tempHeightPropsLast, toolpathZHeightProps &tempHeightPropsCurrent, toolpathZHeightProps &tempHeightPropsSaved, toolpathZHeightProps &masterHeightProps)
{
	// These are copied from regular TPIF::calcSurfInfo(...)
	bool directionDown = false;
	if (masterHeightProps.localAmplToolpath.myFormingDirectionType == amplToolpath::formingDirectionType::negZDir)
		directionDown = true;
...
void TPIF::getSpiralPoints(const toolpathZHeightProps &tempHeightPropsLast, const toolpathZHeightProps &tempHeightPropsCurrent, const toolpathZHeightProps &masterHeightProps, toolpathZHeightProps &tempHeightPropsSaved)
{
...
bool TPIF::correctSphericalInterference(bool topTool, const std::vector<gp_Pnt> &toolCenterPntsIn, const std::vector<gp_Pnt> &toolTipPntsIn, const std::vector<gp_Pnt> &contactPntsIn, const std::vector<gp_Vec> &contactPntNormals,
...
bool TPIF::correctFlatTopInterference(bool topTool, const std::vector<gp_Pnt> &toolCenterPntsIn, const std::vector<gp_Pnt> &toolCornerCenterPntsIn, const std::vector<gp_Pnt> &toolTipPntsIn, const std::vector<gp_Pnt> &contactPntsIn,
...
void TPIF::findLocalTangentDirection(const gp_Pnt &curContactPnt, int curContactPntIndex, const std::vector<gp_Pnt> &contactPnts, const std::vector<gp_Vec> &contactPntNormals, bool reverseNormal, bool goingCCW, gp_Dir &tangentDirOut)
{
	gp_Dir localTanDir;
...
bool TPIF::distPairCompare(const std::pair<float, int> &firstElem, const std::pair<float, int> &secondElem)
{
	return firstElem.first < secondElem.first;
}

void TPIF::removeInitialPnts(const int numPntsToSkip, std::vector<gp_Pnt> &modifiedPntsInOut)
{
	if (numPntsToSkip <= 0)
		return;
...
void TPIF::correctConstZHeightTips(bool isTopTool, const toolpathZHeightProps &masterHeightProps, float sphereRad, const std::vector<gp_Pnt> &contactPnts, const std::vector<gp_Pnt> &toolCenterPnts,
	...
	for (contourCounter = 0; contourCounter < numContours; ++contourCounter)
	{
		...
		for (int m = 0; m < localContactPnts.size(); m++)
		{
			...
			if (directionDown) // Ensure the "new" local normal is in the same direction as the real local normal
				translationVec1 = calcLocalNormal(toolLimitAngle, curNormXY, false, true); // TPIF assumes pos-Z normal for direction down
			else
				translationVec1 = calcLocalNormal(toolLimitAngle, curNormXY, false, false); // TPIF assumes neg-z normal for direction up
...
float TPIF::calcWallAngle(const gp_Vec &localNormalVec)
{
...
gp_Vec TPIF::calcLocalNormal(float wallAngle, const gp_Vec &localNormalXY, bool useReverseNormalXY, bool usePosZDirection)
{
