...
bool DSIF::initContactPnts(amplToolpath &myCurrentToolpathIn, TopoDS_Compound &myCurrentTopContactPntsIn, TopoDS_Compound &myCurrentBotContactPntsIn,
...
bool DSIF::initToolTipPnts(amplToolpath &myCurrentToolpathIn, TopoDS_Compound &myCurrentTopToolTipsIn, TopoDS_Compound &myCurrentBotToolTipsIn,
...
bool DSIF::calcContactPnts()
{
	...
	textEdit->insertPlainText(QString("Initiating DSIF toolpath algorithm. Toolpath rendering will be updated periodically.\n"));
...
bool DSIF::calcToolTipPnts(const bool checkToolpathInterference, const int performConstZHeightTips)
{
...
bool DSIF::toolTipPreprocessing(toolpathZHeightProps &masterHeightProps)
{
...
void DSIF::getToolTipPoints(const toolpathZHeightProps &masterHeightProps, toolpathZHeightProps &tempHeightPropsSaved, bool checkForInterference, int performConstZHeightTips)
{
    ...
    // Currently, the dialog forces this option to be hemispherical tools only
	
	if (performConstZHeightTips == 2 || performConstZHeightTips == 1) // Top tool
	{
	...
	if (performConstZHeightTips == 2 || performConstZHeightTips == -1) // Bottom tool
	{
...
void DSIF::refreshGraphics(const toolpathZHeightProps &allHeightProps, bool refreshContactPnts, bool refreshToolTipPnts)
{
...
void DSIF::removeCollinearPnts(Standard_Real relAngleTolDeg, std::vector<gp_Pnt> &orderdPntsArrInOut, std::vector<int> &indicesKept)
{
...
bool DSIF::toolpathPreprocessing(toolpathZHeightProps &masterHeightProps, std::vector<float> &allZDepthSlices)
{
	float unAssignedVal = myCurrentToolpath->getUnassignedVal();
	if (myCurrentToolpath->topContactPntsActive())
...
bool DSIF::findNextAdaptiveZSlice(const toolpathZHeightProps &tempHeightPropsCurrent, const toolpathZHeightProps &masterHeightProps, float &nextZHeightOut)
{
...
bool DSIF::getDiscretizedIntersection(ampl::planeIntersecter &contourObj, toolpathZHeightProps &tempHeightPropsCurrent, const toolpathZHeightProps &masterHeightProps)
{
	float currentHeight = tempHeightPropsCurrent.currentZDepth;
...
void DSIF::calcSurfInfo(ampl::planeIntersecter &contourObj, toolpathZHeightProps &tempHeightPropsSaved, toolpathZHeightProps &masterHeightProps)
{
...
void DSIF::calcApproxSpiralSurfInfo(ampl::planeIntersecter &contourObj, toolpathZHeightProps &tempHeightPropsLast, toolpathZHeightProps &tempHeightPropsCurrent, toolpathZHeightProps &tempHeightPropsSaved, toolpathZHeightProps &masterHeightProps)
{
	// These are copied from regular DSIF::calcSurfInfo(...)
	toolpathZHeightProps tempHeightPropsSavedCopy;
	bool directionDown = false;
	if (masterHeightProps.localAmplToolpath.myFormingDirectionType == amplToolpath::formingDirectionType::negZDir)
		directionDown = true;
...
void DSIF::getSpiralPoints(const toolpathZHeightProps &tempHeightPropsLast, const toolpathZHeightProps &tempHeightPropsCurrent, const toolpathZHeightProps &masterHeightProps, toolpathZHeightProps &tempHeightPropsSaved)
{
...
bool DSIF::correctSphericalInterference(bool topTool, const std::vector<gp_Pnt> &toolCenterPntsIn, const std::vector<gp_Pnt> &toolTipPntsIn, const std::vector<gp_Pnt> &contactPntsIn, const std::vector<gp_Vec> &contactPntNormals,
...
bool DSIF::correctFlatTopInterference(bool topTool, const std::vector<gp_Pnt> &toolCenterPntsIn, const std::vector<gp_Pnt> &toolCornerCenterPntsIn, const std::vector<gp_Pnt> &toolTipPntsIn, const std::vector<gp_Pnt> &contactPntsIn,
...
void DSIF::findClosestPntsBetweenContours(const std::vector<gp_Pnt> &contourPntsIn, const std::vector<gp_Pnt> &pntsToSearch, std::vector<gp_Pnt> &closestPntsOut)
{
...
void DSIF::findLocalTangentDirection(const gp_Pnt &curContactPnt, int curContactPntIndex, const std::vector<gp_Pnt> &contactPnts, const std::vector<gp_Vec> &contactPntNormals, bool reverseNormal, bool goingCCW, gp_Dir &tangentDirOut)
{
	gp_Dir localTanDir;
...
bool DSIF::distPairCompare(const std::pair<float, int> &firstElem, const std::pair<float, int> &secondElem)
{
	return firstElem.first < secondElem.first;
}

void DSIF::removeInitialPnts(const int numPntsToSkip, std::vector<gp_Pnt> &modifiedPntsInOut)
{
	if (numPntsToSkip <= 0)
		return;
...
void DSIF::correctConstZHeightTips(bool isTopTool, const toolpathZHeightProps &masterHeightProps, float sphereRad, const std::vector<gp_Pnt> &contactPnts, const std::vector<gp_Pnt> &toolCenterPnts,
	...
	for (contourCounter = 0; contourCounter < numContours; ++contourCounter)
	{
		...
		for (int m = 0; m < localContactPnts.size(); m++)
		{
			...
			if (directionDown) // Ensure the "new" local normal is in the same direction as the real local normal
				translationVec1 = calcLocalNormal(toolLimitAngle, curNormXY, false, true); // DSIF assumes pos-Z normal for direction down
			else
				translationVec1 = calcLocalNormal(toolLimitAngle, curNormXY, false, false); // DSIF assumes neg-z normal for direction up

			if (directionDown && !isTopTool) // Specify correct orientiation if correcting the supporting tool in DSIF
				translationVec1.Reverse();
			else if (!directionDown && isTopTool)
				translationVec1.Reverse();
		...
		for (int m = 0; m < localContactPnts.size(); m++)
		{
			...
			else // Find radial component of translation
			{
				...
				if (isTopTool) // Top tool
				{
					gp_Dir xyTransDir;
					if (!directionDown)
						xyTransDir = gp_Dir(curNormXY.X(), curNormXY.Y(), 0.0);
					else
						xyTransDir = gp_Dir(-curNormXY.X(), -curNormXY.Y(), 0.0);
...
float DSIF::calcWallAngle(const gp_Vec &localNormalVec)
{
...
gp_Vec DSIF::calcLocalNormal(float wallAngle, const gp_Vec &localNormalXY, bool useReverseNormalXY, bool usePosZDirection)
{
